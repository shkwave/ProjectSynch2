	...
	if (WORLD.get_state() == "Action_Specifics"):
		if (ActSelReq.flag > 0):
			...
			for tile in ActSelReq.requesting_action.global_selectable_area:
				draw_rect(Rect2(TERRAIN.map_to_world_centered(tile)-rectsize/2,rectsize),Color(1,0,0,0.3))
		if (ActSelReq.flag == 2):
			...
			for tile in ActSelReq.requesting_action.AoE:
				draw_rect(Rect2(TERRAIN.map_to_world_centered(tile+ActSelReq.reply)-rectsize/2,rectsize),Color(0,1,1,0.9))
